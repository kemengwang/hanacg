---
title: ChattyPlay Goofish Backend
emoji: 🐟
colorFrom: blue
colorTo: yellow
sdk: docker
...
闲鱼助手采用双部署：React 前端部署到 Vercel，常驻 Node/Hono 后端部署为 Hugging Face Docker Space。Space 提供 HTTPS/WSS 二级域名，不需要自行购买域名。完整步骤见 [Hugging Face Docker Space 部署指南](docs/huggingface-space.md)。

未配置 Space 地址时，Vercel 上的闲鱼页面会自动进入本地运行模式，不会请求无效接口；配置 `VITE_GOOFISH_API_URL` 后会自动启用远程 API 和 WebSocket。
