...
import { isAllowedFrontendOrigin, resolveCorsOrigin } from '../goofish/core/frontend-origin'
...
import { goofishAuthMiddleware, isAuthorizedGoofishRequest } from '../goofish/api/middlewares'
...
// CORS 配置
app.use('/*', cors({
  origin: isDev ? '*' : (origin) => resolveCorsOrigin(origin),
...
// WebSocket 不受浏览器 CORS 机制保护，需要单独校验 Origin。
app.use('/ws', async (c, next) => {
  const origin = c.req.header('Origin')
  if (!isDev && (!origin || !isAllowedFrontendOrigin(origin) || !isAuthorizedGoofishRequest(c))) {
    return c.json({ error: 'Forbidden WebSocket request' }, 403)
  }
  return next()
})

// Space 后端公开在 *.hf.space，所有业务 API 必须验证站点 JWT。
app.use('/api/*', goofishAuthMiddleware)
