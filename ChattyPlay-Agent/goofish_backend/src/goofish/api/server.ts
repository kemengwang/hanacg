...
import { isAllowedFrontendOrigin, resolveCorsOrigin } from "../core/frontend-origin";
import { goofishAuthMiddleware, isAuthorizedGoofishRequest, securityMiddleware } from "./middlewares";
import {
  createAccountRoutes,
  createGoodsRoutes,
  createMessageRoutes,
...
export function createApp() {
  ...
  app.use("*", cors({
    origin: ENV.IS_DEV ? "*" : (origin) => resolveCorsOrigin(origin),
  ...
  app.use("/ws", async (c, next) => {
    ...
    if (!ENV.IS_DEV && (!origin || !isAllowedFrontendOrigin(origin) || !isAuthorizedGoofishRequest(c))) {
